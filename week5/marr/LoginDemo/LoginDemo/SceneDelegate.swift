...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let windowScene = scene as? UIWindowScene {
            ...
            loginViewController.tabBarItem = UITabBarItem(title: "Login", image: UIImage(systemName: "lock"), tag: 1)
            profileViewController.tabBarItem = UITabBarItem(title: "Profile", image: UIImage(systemName: "person.fill"), tag: 2)
            
            let controllers = [homeViewController, loginViewController, profileViewController]
//            tabBarController.viewControllers = controllers.map {
//                UINavigationController(rootViewController: $0)
//            }
            tabBarController.setViewControllers(controllers.map {
                UINavigationController(rootViewController: $0)
            }, animated: true)
